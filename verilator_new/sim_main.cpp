...
LONG frame_time = 0;
float fps = 0.0;
static int batchSize = 25000000 / 100000;
unsigned char rgb[3];
bool prev_vsync = 0;
int frame_count = 0;
bool prev_hsync = 0;
...
int main(int argc, char** argv, char** env) {
	...
	while (!done)
	{
		...
		ImGui::SetWindowSize("Virtual Dev Board v1.0", ImVec2(600, 600), ImGuiCond_Once);
...
#endif
		if (run_enable) for (int step = 0; step < batchSize; step++) verilate();	// Simulates MUCH faster if it's done in batches.
